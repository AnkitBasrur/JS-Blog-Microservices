...
(async () => {
  ...
  await exec(
    `zip -r ${zipPath} . --exclude terraform/\\* -x node_modules/\\* -x \\*/node_modules/\\* -x \\*/.cache/\\* -x .git/\\* -x \\*.DS_Store`,
    { cwd: rootDir, maxBuffer: 100* 1024 * 1024 }
  );

  console.log("Uploading deployment file...");
  await s3Client
    .putObject({
      Body: fs.createReadStream("C:/Ankit/microservices-demo/tmp/"+filename),
